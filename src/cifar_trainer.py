...
class Cifar10Trainer(object):
    ...
    def train_step(self, images, labels, session):
        loss, summaries, train_op = session.run([self.loss, self.summaries, self.train_op], feed_dict={self.ph_images: images, self.ph_labels: labels})
        return (loss, summaries, train_op)

    def test_step(self, images, labels, session):
        acc, summ = session.run([self.accuracy, self.test_summary], feed_dict={self.ph_images: images, self.ph_labels: labels})
        return acc, summ
